...
export const ChangeRecoverPasswordPage = () => {
  ...
  // The reset token is passed via location state — never in the URL.
  // It's a one-time credential returned by the OTP validation step.
  ...
  useEffect(() => {
    if (!resetToken) {
      toast.error('Invalid or expired reset link. Please start the recovery process again.');
      navigate('/');
    }
  }, [resetToken, navigate]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    changePassword({ newPassword, confirmPassword, resetToken });
  };
