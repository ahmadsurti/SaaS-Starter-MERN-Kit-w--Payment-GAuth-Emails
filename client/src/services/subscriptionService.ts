...
export const subscriptionService = {
  ...
  async getSubscriptionStatus(): Promise<SubscriptionStatus> {
    const response = await axios.get<SubscriptionStatus>('/api/subscriptions/status');
    return response.data;
  },

  // Create customer portal session (handles cancellation, plan changes, billing info)
  async createPortalSession(): Promise<PortalSessionResponse> {
    const response = await axios.post<PortalSessionResponse>('/api/subscriptions/create-portal-session');
    return response.data;
  },
};
