...
import { validateSignup } from '../../types/interfaces';
...
export class SignUpUserValidator implements validateSignup {
  ...
  validatePasswordForce() {
    ...
    if (!this.hasSymbol.test(this.password)) {
      this.errors.push('Password should have at least one special character');
      this.validated = false;
    }
  ...
  checkIfPasswordsAreEqual() {
    if (this.password !== this.confirmPassword) {
      this.errors.push('Password should be equal');
      this.validated = false;
    }
  }
